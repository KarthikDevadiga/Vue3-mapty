  ...
  <div class="container-2">
    ...
    <div class="chat" :class="{ active: chatActive }" @click="toggleChat($event)">
      ...
      <div class="chat-window" :class="{ growing: chatActive, shrinking: !chatActive }">
        ...
        <div
          v-for="Cuser in users"
          :key="user"
          v-show="Cuser.name != user.name"
          class="chat-window__profile"
          @click.prevent="chatWithser(Cuser)"
        >
    ...
    <div class="chating-div" :class="{ 'chating-div-display': chatingActive }">
      ...
      <div class="chat-content">
        <div class="chat-from" v-for="info in chatInfoFrom" :key="info.time">
          <span>{{ info.msg }}</span>
        </div>

        <div class="chat-to" v-for="info in chatInfoTo" :key="info.time">
          <span>{{ info.msg }}</span>
        </div>
      </div>

      <form class="input-div" @submit.prevent="onSend">
...
export default {
  ...
  data() {
    return {
      ...
      currentChatingUser: '',
      chatInfoFrom: [
        { msg: 'Hello', time: '01:25' },
        { msg: 'Are you there ?', time: '02:35' }
      ],
      chatInfoTo: [],
      message: ''
    }
  ...
  methods: {
    ...
    onSend() {
      this.chatInfoTo.push({ msg: this.message, time: '11:01' })
      this.message = ''
    },
    toggleChat(event) {
      console.log(event)
